...
public class ExternalFunction extends SqlFunction {
     ...
     * ImplicitCastOperandTypeChecker. We do not want to allow implicit casts for
     * these operands.
     */
    private record ExactTypeNameChecker(SqlTypeName typeName) implements SqlSingleOperandTypeChecker {
        ...
        public boolean checkSingleOperandType(SqlCallBinding callBinding,
            ...
            final RelDataType operandType = callBinding.getValidator().getValidatedNodeType(operand);
    ...
    public ExternalFunction(SqlIdentifier name, @Nullable RelDataType returnType,
    ...
    public DBSPFunction getImplementation(TypeCompiler typeCompiler, DBSPCompiler compiler) {
        ...
        if (this.getName().toLowerCase().startsWith("jsonstring_as_")) {
            /*
             * TYPE ~ struct
             * pub fn JSONSTRING_AS_<TYPE>(_pos: &SourcePositionRange, s: Option<String>) ->
             * Option<Tup1<TYPE>> {
             * let s: String = (s?);
            ...
            DBSPLetStatement json = new DBSPLetStatement("json",
                    new DBSPApplyExpression("try_parse_json", DBSPTypeAny.getDefault(), param.asVariable())
                            .question());
            statements.add(json);
            DBSPExpression toStruct = new DBSPApplyExpression("try_json_as", structType.setMayBeNull(true),
                    json.getVarReference());
            ...
            // DBSPExpression ok = new DBSPApplyMethodExpression("Ok", new
            // DBSPTypeResult(returnType), retval);
