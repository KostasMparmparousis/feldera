...
CREATE FUNCTION getEmployeesByDepartment(id INT) AS (
  ((SELECT EmployeeID, FirstName, LastName, JobTitle, Salary, Status FROM Employees WHERE Status = 'Active') UNION (SELECT EmployeeID, FirstName, LastName, JobTitle, Salary, Status FROM Managers WHERE Status = 'Active') ORDER BY LastName)
);
